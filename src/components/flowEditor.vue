<template>
  <div class="container">
    <el-row class="mb-4">
      <el-button type="primary" @click="resetTransform" style="color: black"
        >重置</el-button
      >
      <el-button type="primary" @click="updatePos" style="color: black"
        >修改属性</el-button
      >
      <el-button type="primary" @click="toggleclass" style="color: black"
        >修改样式</el-button
      >
      <el-button type="primary" @click="logToObject" style="color: black"
        >查看属性</el-button
      >
    </el-row>
    <div id="flowPanel">
      <VueFlow fit-view-on-init class="my-flow" v-model="elements">
        <Background />
        <sideBar />
        <Controls />
      </VueFlow>
    </div>
  </div>
</template>

<script lang="ts" setup name="DemoBpmn">
import {} from 'vue';
import '@vue-flow/core/dist/style.css';
/* import the default theme (optional) */
import '@vue-flow/core/dist/theme-default.css';

import { Background, Controls } from '@vue-flow/additional-components';
import { VueFlow, useVueFlow } from '@vue-flow/core';
import { ref } from 'vue';
import { ElMessage } from 'element-plus';
import sideBar from './vueFlow/sideBar.vue';

const data = [
  {
    id: '1',
    type: 'input',
    label: '敏感性分析',
    position: { x: 250, y: 5 },
  },
  { id: '2', label: 'Node 2', position: { x: 100, y: 100 } },
  { id: '3', label: 'Node 3', position: { x: 400, y: 100 } },
  { id: '4', label: 'Node 4', position: { x: 400, y: 200 } },
  { id: 'e1-2', source: '1', target: '2', animated: true },
  { id: 'e1-3', source: '1', target: '3', animated: true },
  { id: 'e1-4', source: '3', target: '4', animated: true },
];

let elements = ref(data);

let {
  onPaneReady,
  onNodeDragStop,
  onConnect,
  addEdges,
  setTransform,
  toObject,
  nodes,
} = useVueFlow();

onPaneReady(({ fitView }) => {
  fitView();
});
onNodeDragStop((e) => console.log('drag stop', e));
onConnect((params) => addEdges([params]));

const updatePos = () => {
  nodes.value.forEach((el) => {
    el.position = {
      x: Math.random() * 400,
      y: Math.random() * 400,
    };
  });
};

const logToObject = () => {
  ElMessage.info(JSON.stringify(toObject()));
};
const resetTransform = () => {
  elements.value = data;
  setTransform({ x: 0, y: 0, zoom: 1 });
};
const toggleclass = () =>
  nodes.value.forEach(
    (el) => (el.class = el.class === 'node-light' ? 'node-dark' : 'node-light')
  );
</script>

<style scoped>
.my-flow {
  margin: 10px;
  height: 400px;
}

.my-flow .node-light {
  background: none;
}

.my-flow .node-dark {
  background: #eeeeee;
}

#flowPanel {
  border-width: 1px;
  border-color: black;
  border-radius: 7px;
}

.container {
  margin: 40px 40px;
}
</style>
